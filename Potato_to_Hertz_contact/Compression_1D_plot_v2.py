...
# manim Compression_1D_plot_v2.py --disable_caching -p
...
class TMCCompression1D(Scene):
    def construct(self):
        ...
        axes = Axes(x_range=[0, 1.04, 5], y_range=[0, plot_y_scale*y_length,5], x_length=x_length, y_length=y_length, color=BLACK,axis_config={'tip_shape': StealthTip,'tip_width':0.2,'tip_height':0.2,'color':BLACK,'stroke_color':BLACK,'fill_color':BLACK,'stroke_width':4}).shift(pos_plot)
        func = axes.plot(lambda x: math.log(x)**2/y_log_scaling, x_range=[x_ini, 1, 0.001],stroke_width=6, color=GRAY_C)
        ...
        dot_0 = Dot(point= point_input[0,:], color="#dc7900")
        dot_0.set_z_index(3)

        # my_color = Color(rgb=[220/255, 120/255, 0/255])
        ...
        load = VGroup()
        for i in range(3):
            load += Arrow(start=i*UP/3.3+RIGHT, end=i*UP/3.3,stroke_width=10,max_stroke_width_to_length_ratio=10,max_tip_length_to_length_ratio=0.4,color="#dc7900")
        load.next_to(compression_bar,aligned_edge=RIGHT)#.shift((1-3/3.3)*DOWN)

        bar_trace_line = Line(0*UP,5*DOWN,color="#dc7900",stroke_width=1.2).align_to(compression_bar,UP+RIGHT).shift(0.5*UP)

        move_bar = VGroup(compression_bar,load,bar_trace_line)
        compression_bar_copy = DashedVMobject(compression_bar.copy(), dashed_ratio=0.5 )
        compression_bar_copy.set_fill(opacity=0)
        compression_bar_copy.set_stroke( width=5, color=GRAY_B)
        # compression_bar_copy.set_style(stroke_dasharray=[2, 1])

        self.play(FadeIn(compression_bar,cover_bar,support,load,bar_trace_line,dot_0,compression_bar_copy))
